      ...
      <AppShell title="Home" identity={identity} csrf={csrf} current="home">
            ...
            <div mix={reveal}>
              <Stub>
                <div mix={inventoryStubBody}>
                  <Money cents={pnl.inventoryCents} tone="flat" size="lg" block />
        ...
        {pnl.slices.length > 0 ? (
          <section mix={sectionBlock}>
            <SectionLabel>Tag slices</SectionLabel>
            <ul mix={[sliceGrid, revealStagger]}>
              {pnl.slices.map((slice) => (
                <SliceCard key={slice.untagged ? 'untagged' : slice.name} slice={slice} />
              ))}
            </ul>
          </section>
        ) : null}
...
function ProfitStamp(handle: {
  ...
  return () => {
    ...
    return (
      <a href={href} mix={profitStampLink} aria-current={selected ? 'true' : undefined}>
        <Stub>
...
function SliceCard(handle: { props: { slice: TagSlice } }) {
  return () => {
    ...
    let profitTone =
      slice.profitCents > 0 ? sliceProfitGain : slice.profitCents < 0 ? sliceProfitLoss : undefined
...
const sliceCard = css({
  ...
  minWidth: 0,
  boxShadow: '0 1px 0 rgba(255,255,255,0.6) inset, 0 6px 14px -10px rgba(34,26,18,0.55)',
